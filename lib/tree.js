...
const { getPoseidon, initPoseidon } = require("./crypto");
...
class MerkleTreeBuilder {
  ...
  async init() {
    if (!this.poseidon) {
      console.log("🔧 Initializing Poseidon hash (shared instance)...");
  ...
  /**
   * Build sparse Merkle tree (memory-efficient for < 10k leaves)
   * Uses Map instead of Array to only store non-zero nodes
   * @private
   */
  async buildSparseTree(chunkId, currentLeaves) {
    const numLeaves = currentLeaves.length;
    const start = Date.now();
    
    // Use Map for sparse storage (only non-zero nodes)
    const tree = new Map();
    
    // 1. Fill leaf nodes
    for (let i = 0; i < numLeaves; i++) {
      tree.set(i, currentLeaves[i]);
    }
    
    // 2. Build intermediate nodes level by level
    let nodeOffset = 0;
    ...
    for (let level = 0; level < TREE_HEIGHT; level++) {
      ...
      for (let i = 0; i < currentLevelNodeCount; i += 2) {
        ...
        // Get nodes from sparse storage (default to ZERO/ZERO_HASHES if not present)
        const left = tree.get(leftIndex) || ZERO;
        const right = tree.get(rightIndex) || (level === 0 ? ZERO : ZERO_HASHES[level - 1]);
        
        // Skip hashing if both are default values (optimization)
        if (left === ZERO && right === (level === 0 ? ZERO : ZERO_HASHES[level - 1])) {
          // Result would be ZERO_HASHES[level], don't store it
          continue;
        }
        
        const parentIndex = parentLevelOffset + Math.floor(i / 2);
        const hash = await this.poseidon([left, right]);
        tree.set(parentIndex, BigInt(this.poseidon.F.toObject(hash)));
      }
    ...
    const root = tree.get(treeSize - 1) || ZERO_HASHES[TREE_HEIGHT - 1];
    const elapsed = ((Date.now() - start) / 1000).toFixed(2);
    
    console.log(`✅ Sparse tree built in ${elapsed}s`);
    console.log(`   Root: ${root.toString().substring(0, 20)}...`);
    console.log(`   Memory: ${tree.size} nodes stored (vs ${treeSize} in full tree)`);
    
    // Convert Map to Array for compatibility with existing code
    // Only convert accessed indices (lazy conversion)
    const fullTree = new Array(treeSize).fill(ZERO);
    for (const [index, value] of tree.entries()) {
      fullTree[index] = value;
    }
   ...
   * 
   * PERFORMANCE: Automatically uses sparse tree for < 10k leaves
   * to save memory (64MB → ~100KB for typical cases)
   * 
  ...
  async buildTree(chunkId, currentLeaves) {
    ...
    // PERFORMANCE OPTIMIZATION: Use sparse tree for small leaf counts
    // Saves 99%+ memory (64MB → ~100KB for < 10k leaves)
    const SPARSE_THRESHOLD = 10000;
    if (numLeaves < SPARSE_THRESHOLD) {
      console.log(`   Using sparse tree (saves ~${((1 - numLeaves / LEAVES_CAPACITY) * 100).toFixed(1)}% memory)`);
      return await this.buildSparseTree(chunkId, currentLeaves);
    }

    // Full tree for large datasets
    const start = Date.now();
    const treeSize = 2 ** (TREE_HEIGHT + 1) - 1;
