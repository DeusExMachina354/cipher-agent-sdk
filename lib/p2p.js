...
const MAX_REQUESTS_PER_MINUTE = 100; // Global requests per IP
const MAX_TREE_REQUESTS_PER_MINUTE = 10; // Tree downloads per IP
const MAX_CONCURRENT_CONNECTIONS = 50; // Total simultaneous connections
const RATE_LIMIT_WINDOW = 60000; // 1 minute
const REQUEST_TIMEOUT = 30000; // 30 seconds max per request
const MAX_PEER_LIST_SIZE = 100; // Max peers to return
...
const DHT_BOOTSTRAP_NODES = [
  // 🚀 Official Bootstrap Node (fly.io)
  { host: 'bootstrap-node.fly.dev', port: 8549 },
...
class TreeP2P {
  constructor(builder, options = {}) {
    ...
    // 🛡️ Rate Limiting & Security
    this.rateLimits = new Map(); // IP => { requests: [], treeRequests: [] }
  ...
  /**
   * 🛡️ Check if IP is rate limited
   * @private
   */
  _isRateLimited(ip, isTreeRequest = false) {
    if (this.bannedIPs.has(ip)) {
      return true; // Permanently banned
    }

    const now = Date.now();
    let limiter = this.rateLimits.get(ip);
    
    if (!limiter) {
      limiter = { requests: [], treeRequests: [] };
      this.rateLimits.set(ip, limiter);
    }

    // Cleanup old entries
    limiter.requests = limiter.requests.filter(t => now - t < RATE_LIMIT_WINDOW);
    limiter.treeRequests = limiter.treeRequests.filter(t => now - t < RATE_LIMIT_WINDOW);

    // Check limits
    if (limiter.requests.length >= MAX_REQUESTS_PER_MINUTE) {
      return true;
    }
    
    if (isTreeRequest && limiter.treeRequests.length >= MAX_TREE_REQUESTS_PER_MINUTE) {
      return true;
    }
  ...
  async startServer() {
    return new Promise((resolve, reject) => {
      this.server = http.createServer((req, res) => {
        ...
        if (treeMatch) {
          // 🛡️ Rate Limit - Tree requests are expensive
          if (this._isRateLimited(clientIP, true)) {
            res.writeHead(429, { "Content-Type": "application/json" });
            res.end(JSON.stringify({ 
              error: "Rate limit exceeded",
              limit: MAX_TREE_REQUESTS_PER_MINUTE,
              window: "1 minute"
            }));
            return;
          }
          ...
          // 🛡️ Validate chunk ID
          if (isNaN(chunkId) || chunkId < 0 || chunkId > 1000000) {
            res.writeHead(400, { "Content-Type": "application/json" });
            res.end(JSON.stringify({ error: "Invalid chunk ID" }));
            return;
          }
        ...
        // GET /health - Health check (no rate limit - monitoring tool friendly)
        if (req.url === "/health") {
        ...
        // GET /status - More detailed status (rate limited)
        if (req.url === "/status") {
          // 🛡️ Rate Limit
        ...
        // GET /peers - Return peer list (rate limited + size limited)
        if (req.url === "/peers") {
          // 🛡️ Rate Limit
          if (this._isRateLimited(clientIP)) {
            res.writeHead(429, { "Content-Type": "application/json" });
          ...
          const peerList = Array.from(this.peers.values())
            .slice(0, MAX_PEER_LIST_SIZE) // 🛡️ Limit response size
            .map(p => ({
              host: p.host,
              port: p.port,
              lastSeen: p.lastSeen,
      ...
      this.server.listen(this.port, '0.0.0.0', () => {
        console.log(`🌐 P2P server running on 0.0.0.0:${this.port}`);
        this._startRateLimitCleanup(); // 🛡️ Start cleanup timer
        resolve();
      });
  ...
  /**
   * 🛡️ Start rate limit cleanup timer
   * @private
   */
  _startRateLimitCleanup() {
    setInterval(() => {
      const now = Date.now();
      for (const [ip, limiter] of this.rateLimits.entries()) {
        limiter.requests = limiter.requests.filter(t => now - t < RATE_LIMIT_WINDOW);
        limiter.treeRequests = limiter.treeRequests.filter(t => now - t < RATE_LIMIT_WINDOW);
        
        // Remove empty entries
        if (limiter.requests.length === 0 && limiter.treeRequests.length === 0) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.broadcastSocket.on('message', (msg, rinfo) => {
        try {
          ...
          // Ignore malformed messages (log in DEBUG mode)
          if (process.env.DEBUG) {
            console.debug(`P2P: Malformed broadcast from ${rinfo.address}:`, err.message);
          }
        }
      });
